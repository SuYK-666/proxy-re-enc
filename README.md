...
![Status](https://img.shields.io/badge/Status-security--hardened-success)
...
项目中的 `RSA_PRE` 与 `ECC_PRE` 明确定位为教学型 baseline 和对照实验实现，不作为生产级密码安全承诺。生产路径的核心边界是：服务端保存密文、nonce、AAD、capsule 与元数据，不保存用户明文；正式部署应将 baseline PRE 替换为经过审查的 PRE、Threshold PRE、Umbral-style PRE、HPKE 或成熟密码库方案。
...
| 存储层 | In-memory repository、JDBC schema、快照导出 | `storage/*`、`schema.sql` |
...
- In-memory repository 适合本地演示和测试；真实部署应接入事务型数据库、KMS/HSM 与集中审计系统。
...
| 可验证审计 | hash-chain audit log、tamper detection、proof export 与 anchor 设计 |
| API 加固 | Bearer token、统一错误响应、请求体大小限制、幂等键、基础限流 |
| 持久化设计 | 提供用户、数据、授权、package、nonce、token revoke、audit 表结构 |
| 实验报告 | 保留 L0-L10、攻击测试、安全审查修复、性能 benchmark 等原始输出 |
...
完整路径可从 `/openapi.json` 查看。下面是核心流程。
...
| [docs/reports/performance-results.csv](docs/reports/performance-results.csv) | RSA/ECC baseline 性能基准原始数据 |
| [docs/reports/performance-summary.md](docs/reports/performance-summary.md) | 性能摘要与指标解释 |
...
│   ├── storage             # Repository 接口、内存实现、JDBC schema 初始化
...
- 将 in-memory repository 切换为事务型数据库实现，并启用外键、唯一约束、乐观锁和迁移脚本。

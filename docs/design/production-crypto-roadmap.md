...
This repository keeps RSA-PRE and ECC-PRE as teaching prototypes.
A production-grade deployment should replace the crypto backend behind
`PreScheme` with reviewed libraries.
...
The management layer is intentionally separated from `PreScheme` so the
lifecycle, authorization, audit, benchmark, and API evidence can remain stable
while the algorithm provider is replaced.

...
`crypto/provider/CryptoProvider` 统一提供 key generation、DEK encapsulation、可选 proxy
transform 和 decapsulation；`SchemeDescriptor` 必须公开：
...
`CryptoProviderRegistry.productionDefault()` 只会选择非 baseline 的已实现 provider；
仅存在 baseline 时必须失败。

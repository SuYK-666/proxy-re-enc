...
2. 算法流程完整，便于解释代理重加密的密钥转换过程。
3. 数据加密采用混合加密：代理重加密只处理数据密钥封装，文件内容使用 AES-GCM 加密。
4. RSA 与 ECC 使用统一的业务抽象，便于横向对比。
5. 所有演示流程都能通过自动化测试验证。
...
2. 数据拥有者必须长期在线，授权效率低。
3. 每新增一个被授权人，就可能需要生成一份新的密文副本，存储成本高。
4. 如果直接把数据密钥发给被授权人，密钥泄露后难以控制风险。
5. 云端访问控制和密码学访问控制没有解耦，半可信云平台仍可能成为安全薄弱点。
...
2. 每个被授权人都保存一份独立文件密文，存储和同步成本较高。
3. 若直接把数据密钥发送给 Bob，密钥泄露后云端无法再提供密码学隔离。

PRE 解决方式：

1. 文件内容只加密一次，云端只保存一份文件密文。
2. Alice 只为 Bob 生成重加密密钥，代理只转换数据密钥胶囊。
3. Bob 使用自己的私钥恢复数据密钥，代理和云端都不接触文件明文。

局限性：

1. 授权撤销需要配合密钥轮换或版本化数据密钥。
2. 若被授权人已经获得明文，密码学机制无法阻止其离线复制。
...
2. 患者临时授权医生、保险机构或远程会诊专家访问数据时，若依赖人工导出和重新加密，效率低且审计困难。

PRE 解决方式：

1. 患者或数据管理方作为 Owner 上传加密病历。
2. 当医生 Bob 获得授权时，代理只转换病历数据密钥胶囊。
3. 平台可以记录“谁授权、谁转换、谁访问”，但不能读取病历明文。

局限性：

1. 医疗场景通常要求强身份认证、合规审计和细粒度撤销，课程原型只演示密码学核心流程。
2. 生产系统还需要密钥托管、硬件安全模块和合规访问策略。
...
2. 自动转发规则与端到端加密天然冲突，因为邮件服务器不应读取明文。

PRE 解决方式：

1. Alice 预先向邮件代理提供 Alice 到 Bob 的重加密密钥。
2. 邮件服务器在不解密邮件内容的前提下，把邮件密钥胶囊转换给 Bob。
3. Bob 用自己的私钥读取邮件，服务器仍然不知道邮件明文。

局限性：

1. 邮件元数据仍可能暴露通信关系。
2. 生产系统需要防止代理滥用长期重加密密钥。
...
2. 智能合约适合记录授权状态，但不适合处理明文或大文件加密。

PRE 解决方式：

1. 数据密文保存在 IPFS、对象存储或链下数据库。
2. 链上只记录授权关系、数据哈希和访问策略。
3. 代理根据授权事件转换数据密钥胶囊，避免重新上传大文件。

局限性：

1. 链上授权撤销与链下密钥轮换需要一致性设计。
2. 代理节点的可用性和作恶检测需要额外机制。

### 2.4 PRE 理论分类

为了满足“深入剖析”的要求，报告中会明确讨论 PRE 的以下分类，并把本项目实现放入这些分类中。

1. 单向 PRE 与双向 PRE：单向 PRE 只允许 Alice 到 Bob 的转换，不能自动推出 Bob 到 Alice 的转换；双向 PRE 则可以双向转换，但通常意味着更强的密钥关联和更高的安全风险。本项目 RSA 与 ECC 教学方案都按单向授权流程组织。
2. 单跳 PRE 与多跳 PRE：单跳 PRE 只允许密文转换一次；多跳 PRE 允许 A 到 B 再到 C 的连续转换。本项目只实现单跳，避免多次转换带来的安全分析复杂度。
3. 交互式 PRE 与非交互式 PRE：非交互式方案中，Owner 只使用自己的私钥和 Recipient 公钥生成转换密钥；交互式方案需要 Recipient 参与授权协议。RSA 教学方案接近非交互式；ECC 教学方案由于公式限制，需要把 Recipient 的参与作为安全边界重点说明。
4. 单代理 PRE 与门限 PRE：单代理方案把转换能力交给一个代理；门限 PRE 将转换密钥拆成多个片段，需要多个代理共同完成转换。生产级 Umbral 类方案常采用门限思想，本项目实现单代理版本，报告中作为生产扩展讨论。
5. 代理透明性：代理透明性关注接收者是否能区分密文是否被代理转换过，以及代理是否能识别授权关系。本项目不实现代理透明性，只记录审计日志并用于课程演示。
...
2. 数据使用者 Recipient：获得转换后的密文并解密数据。
3. 代理 Proxy：半可信，负责密文转换。
4. 云存储 Storage：保存密文数据和元数据。
5. 攻击者 Attacker：可能窃取云端密文、监听网络、尝试伪造授权。

### 3.2 安全目标

1. 云存储无法读取数据明文。
2. 代理仅凭重加密密钥和密文无法恢复明文。
3. 未被授权用户无法解密数据。
4. 被授权用户只能通过自己的私钥解密转换后的密文。
5. 系统可以记录授权、转换、解密等关键事件。

### 3.3 非目标与限制

1. 本项目不实现生产级密钥管理系统。
2. 本项目不实现分布式云存储。
3. RSA 方案采用教学型共同模数 RSA-PRE KEM，并加入共享指数因子以避免最直观的共同模数攻击，但仍不声称具备生产级 CCA 安全。
4. ECC 方案采用教学型椭圆曲线 ElGamal 风格交互式 PRE KEM，不要求 Alice 直接知道 Bob 私钥，但需要 Bob 参与生成转换材料；该方案不声称抵抗代理与被授权人合谋。
5. 若要生产使用，应改用经过安全证明和工程审计的成熟代理重加密方案，例如 Umbral 类方案或基于双线性配对的 PRE 方案。
...
业务层只统一“封装、解封装、代理转换”等通用动作，不强行统一“重加密密钥生成”签名。原因是 RSA 与 ECC 的 reKey 生成假设不同：RSA 教学方案可由 Owner 根据自己的私钥和 Recipient 公钥生成；ECC 教学方案需要 Recipient 参与生成转换材料。把二者硬塞进同一个方法会污染接口，并掩盖 ECC 方案的安全边界。
...
说明：`RecipientReKeyShare` 是 ECC 教学方案的显式交互材料，不能在报告中描述成“Bob 私钥交给 Alice”。实现时会把它标记为敏感材料，并在安全分析中说明它仍然不足以达到生产级抗合谋安全。生产级方向采用 Umbral 类或配对型 PRE，不使用该教学接口。
...
3. 使用 PRE KEM 将 DEK 封装给数据拥有者。
4. 上传文件密文、AES-GCM nonce、认证标签、PRE 胶囊和元数据。
5. 授权时只转换 PRE 胶囊，不重新加密文件内容。
6. 被授权人解封装 DEK，再用 DEK 解密文件密文。
...
这就是共同模数攻击。由于代理在重加密过程中可能同时看到原始胶囊和转换后胶囊，若不处理该问题，代理就能恢复 KEM 随机数 s，进而恢复数据密钥。因此，本项目实验实现要求所有用户公开指数共享同一个大因子 h，使攻击者最多由上述方法恢复 `s^h`，不能直接得到 s。
...
2. 默认 RSA 模数长度建议为 2048 位，可扩展到 3072 位。
3. 生成系统级共享指数因子 h，并保证所有用户公钥指数都含有 h。
4. 测试中显式验证 `gcd(e_A, e_B) = h` 或至少大于 1，避免朴素共同模数攻击。
5. 随机 s 必须满足 1 < s < N 且 gcd(s, N) = 1。
6. 所有字节编码使用固定长度无符号大端格式。
7. 性能统计记录 keygen、encapsulate、rekey、reencrypt、decapsulate 五个阶段。
...
2. 报告分析层：正式说明该教学方案的缺陷，并给出生产级方向，例如 Umbral 类门限 PRE 或基于双线性配对的 PRE。生产级方向要求 reKey 生成只依赖 Owner 私钥和 Recipient 公钥，不能要求 Recipient 私钥离开持有人。
...
2. 课程实现中，Bob 通过本地方法参与授权会话，生成转换所需的敏感交互材料；代码接口不再把 `recipientPrivateKey` 放进通用 `PreScheme`。
3. 即使不直接传递 Bob 私钥，最终转换因子在数学上仍与 `x_A * x_B^{-1}` 等价。
4. 若代理拿到 `rk_A_to_B` 后与 Bob 合谋，Bob 可以结合自己的 x_B 推出 Alice 的 x_A。
5. 因此该 ECC 方案只能作为“交互式、非抗合谋”的教学型 PRE，不能作为生产级方案。
...
2. 若部署环境允许引入 Bouncy Castle，后续可替换底层点运算实现。
3. 验证输入点是否在曲线上，拒绝无穷远点作为外部输入。
4. 标量运算统一 mod n。
5. 点编码使用未压缩 SEC1 格式：0x04 || x || y。
6. KDF 输入使用完整点编码，而不是只使用 x 坐标。
7. 不把教学型点运算描述为常数时间实现，不声称具备侧信道安全。
8. 性能统计记录 keygen、encapsulate、rekey、reencrypt、decapsulate 五个阶段。
...
5. AES/GCM/NoPadding。
6. SHA-256 或 HKDF-SHA256。
7. 可选：Bouncy Castle，用于替换教学型 ECC 点运算。
8. 可选：JMH，用于更严谨的性能测试。若课程环境不方便引入依赖，则实现轻量级基准测试器。
...
1. Owner 能解密自己加密的数据。
2. Recipient 不能直接解密 Owner 的原始胶囊。
3. Proxy 使用正确 reKey 后，Recipient 能解密转换后的数据。
4. Proxy 不能仅凭密文和 reKey 得到 DEK。
5. 其他未授权用户不能解密转换后的数据。
6. 同一文件内容在加密后密文不等于明文。
7. 多次加密同一明文得到不同密文。
8. RSA 参数测试验证 `gcd(e_A, e_B) > 1`，避免朴素共同模数攻击条件成立。
9. ECC 接口测试验证通用 `PreScheme` 不接收 Recipient 私钥，交互材料只出现在 ECC 专用授权流程中。

### 9.2 篡改检测

1. 修改 AES-GCM 密文，解密应失败。
2. 修改 AES-GCM nonce，解密应失败。
3. 修改 PRE 胶囊，解封装应失败或得到不可通过认证的数据密钥。
4. 使用错误 recipient 私钥，解密应失败。
...
2. ECC P-256 私钥和公钥尺寸较小，点乘性能通常较好。
3. AES-GCM 文件加密耗时随文件大小线性增长。
4. PRE 重加密耗时与文件大小无关，体现混合加密优势。
...
1. 创建 Maven 工程。
2. 配置 Java 17、JUnit 5。
3. 建立 model、crypto、service、storage 包。
4. 建立统一异常类型和工具类。
...
2. 实现共享指数因子 h。
3. 实现用户指数密钥生成。
4. 实现封装、解封装、reKey 生成和重加密。
5. 完成 RSA 正确性、异常测试和共同模数攻击规避测试。

### 阶段 4：ECC-PRE

1. 实现无外部依赖的 P-256 教学型点运算。
2. 实现点编码和点合法性校验。
3. 实现 ECC 封装、解封装、交互式 reKey 生成和重加密。
4. 完成 ECC 正确性、异常测试和抗合谋限制说明。
...
4. 实现代理重加密。
5. 实现被授权人读取。
6. 实现 DemoApplication 输出完整流程。

### 阶段 6：性能评估

1. 实现 BenchmarkService。
2. 采集 RSA 和 ECC 指标。
3. 输出 Markdown 或 CSV 结果。
4. 整理性能对比结论。
...
2. 测试中加入检查，确保 Alice 和 Bob 的 `gcd(e_A, e_B)` 不为 1。
3. 在代码注释、README 和报告中明确标记为教学原型，并解释为什么真实系统不应直接采用共同模数 RSA-PRE。
4. 报告中将该攻击作为“为什么需要成熟 PRE 协议”的核心论据之一。

### 12.2 ECC 教学方案抗合谋不足

风险：ECC 教学公式 `rk_A_to_B = x_A * x_B^{-1} mod n` 清晰但脆弱。代理和 Bob 若合谋，Bob 可以从 rk 和自己的 x_B 恢复 Alice 私钥 x_A。同时，若实现时把 Bob 私钥或等价逆元材料交给 Alice，也违背私钥不离开持有人的基本原则。

应对：

1. 接口层不再把 `recipientPrivateKey` 放进统一 `PreScheme`。
2. ECC reKey 生成被定义为教学型交互式授权会话，由 Bob 本地参与生成敏感转换材料。
3. 报告中明确说明该方案不抗代理与 Recipient 合谋。
4. 把 Umbral 类门限 PRE 和基于双线性配对的 PRE 作为生产级替代方向说明。

### 12.3 椭圆曲线点运算实现复杂

风险：手写点运算容易出现边界条件错误，也容易因为条件分支、异常点处理和标量乘算法暴露时序侧信道。课程实现若完全自研 ECC 底层，评审者可能质疑实现可靠性。

应对：

1. 当前实现启用教学型 P-256 点运算，保证无外部依赖可运行。
2. 若课程环境允许依赖，后续可引入 Bouncy Castle 处理 P-256 点运算。
3. 增加曲线点合法性测试。
4. 用已知基点阶性质测试 n * G = O。
5. 测试 a * (b * G) = b * (a * G)。
6. 明确声明教学型点运算不具备生产级侧信道安全。
...
2. 使用 Maven 作为构建工具。
3. 先实现固定场景 Demo，不做复杂交互式菜单。
4. ECC 当前采用教学型 P-256 点运算；若课程环境允许依赖，可切换到 Bouncy Castle。
5. 性能测试先使用内置轻量 benchmark，不强依赖 JMH。
6. 数据示例同时支持文本和本地文件，加分点优先保留。
...
2. 默认实现固定场景 Demo，同时保留简单 CLI 扩展空间。
3. RSA-PRE 使用 2048 位共同模数教学方案，并为公开指数加入共享大因子 h，避免最简单的共同模数攻击。
4. ECC-PRE 使用 P-256 交互式教学方案，当前使用无依赖教学型点运算，报告中明确它不抗合谋。
5. 文件内容统一使用 AES-256-GCM。
6. 性能测试先实现内置轻量 benchmark，避免额外依赖影响课程环境。
7. README 中单独加入“安全限制与生产替代方案”章节。
